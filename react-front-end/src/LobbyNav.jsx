      ...
      <Route>
        <div className="d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 bg-dark border-bottom shadow-sm">
          ...
          <a className="navbar-a p-2 text-white" style={{color: 'white', textDecoration: 'none'}}>{this.props.userData.username ? `Logged in as: ${this.props.userData.username}` : null}</a>
            <div>
              {this.props.userData.username ? <div><button className="btn btn-dark sign-up-button btn-md nav-signup-button" onClick={this.props.onLogout}>Log Out</button>
              <button onClick={this.props.createRoom} className="btn btn-dark btn-lg create-room-button">Create Room</button></div> 
              : null}
            </div>
          </nav>
        </div>
